...
@interface FlutterUcropPlugin() <TOCropViewControllerDelegate>
@end

@implementation FlutterUcropPlugin{
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel
      methodChannelWithName:@"plugins.com.baiheche/image_crop"
            binaryMessenger:[registrar messenger]];
    UIViewController *viewController = [UIApplication sharedApplication].delegate.window.rootViewController;
    FlutterUcropPlugin* instance = [[FlutterUcropPlugin alloc] initWithViewController:viewController];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  } else if ([@"cropFile" isEqualToString:call.method]){
      _result = result;
      _arguments = call.arguments;
      NSString *sourcePath = call.arguments[@"source_path"];
      NSString *compressFormat = call.arguments[@"compress_format"];
...
- (void)setupUiCustomizedOptions:(id)options forViewController:(TOCropViewController*)controller {
    ...
    NSString *initAspectRatioPreset = options[@"ios.init_aspect_ratio_preset"];
    ...
    if (initAspectRatioPreset) {
        controller.aspectRatioPreset =[self parseAspectRatioPresetFromName:initAspectRatioPreset];
    }else{
        controller.aspectRatioPreset =TOCropViewControllerAspectRatioPresetSquare;
    }
}
...
-(void) cropImage:(NSString *)path{
    UIImage* imageFile = [UIImage imageWithContentsOfFile:path];
    ...
    CGRect rect = CGRectMake(x, y, imageHeight, imageHeight);
    imageFile = [imageFile croppedImageWithFrame:rect angle:0 circularClip:NO];
    imageFile = [self normalizedImage:imageFile];
    NSString *guid = [[NSProcessInfo processInfo] globallyUniqueString];
    NSData *data;
    NSString *tmpFile;
    if ([@"png" isEqualToString:_compressFormat]) {
        data = UIImagePNGRepresentation(imageFile);
        tmpFile = [NSString stringWithFormat:@"image_cropper_%@.png", guid];
    } else {
        data = UIImageJPEGRepresentation(imageFile, 0.9f);
